...
Annealer::Annealer(State& state, RNG& rng, const AnnealConfig& config)
    : state_(state), rng_(rng), config_(config), temp_start_(1.0), temp_end_(0.05), temp_(1.0) {
  calibrate();
  retune(temp_start_);
}
...
AnnealResult Annealer::run() {
  ...
  int best = state_.score();
  auto best_perms = state_.snapshot();
  ...
  while (true) {
    ...
    const bool accept = (delta <= 0) || (rng_.unit() < acceptance(delta));
    if (!accept) {
      state_.apply(m);
      continue;
    }

    if (state_.score() < best) {
      best = state_.score();
      best_perms = state_.snapshot();
      if (best == 0) {
        break;
      }
    }
